...
Power handling combines `SystemEvents.PowerModeChanged`, native `WM_POWERBROADCAST` messages, and `GUID_LIDSWITCH_STATE_CHANGE`. Before lid close, suspend, or hibernation, ProFan stops curve maintenance and restores the captured ASUS firmware profile. If manual mode was active, its percentage is retained only as a resume intent. Separate lid and system-suspend flags prevent an early wake notification from restoring manual control while the lid remains closed. Once both conditions are clear, a bounded timer retries ATKACPI initialization and reapplies that percentage; exhaustion leaves Automatic active.
